...
public sealed class CreditsHunk
{
    ...
    internal static int HunkByteLength(CreditsHunk hunk) =>
        CreditsAsset.HunkHeaderSize + PaddedTextLength(hunk.Text1) + PaddedTextLength(hunk.Text2);

    /// <remarks>
    /// <paramref name="reader"/>'s position is restored before returning; a hunk header's text offsets
    /// are absolute.
    /// </remarks>
    internal static CreditsHunk Read(EndianReader reader, FormatProfile _)
    {
    ...
    /// </remarks>
    internal static void Write(CreditsHunk hunk, EndianWriter writer, FormatProfile _)
    {
        ...
        writer.Write((uint)(CreditsAsset.HunkHeaderSize + text1Length + text2Length));
        writer.Write(hunk.PresetIndex);
        writer.Write(hunk.StartTime);
        writer.Write(hunk.EndTime);
        writer.Write(hunk.Text1 is null ? 0u : (uint)(CreditsAsset.HeaderSize + hunkStart + CreditsAsset.HunkHeaderSize));
        writer.Write(hunk.Text2 is null ? 0u : (uint)(CreditsAsset.HeaderSize + hunkStart + CreditsAsset.HunkHeaderSize + text1Length));
    ...
    private static string? ReadInlineText(EndianReader r, uint offsetFromAssetStart)
    {
        ...
        r.BaseStream.Position = offsetFromAssetStart - CreditsAsset.HeaderSize;
