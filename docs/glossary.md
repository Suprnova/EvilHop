...
| `PACK` | [`Package`](../src/EvilHop/Blocks/Package/Package.cs) | Root of the archive's metadata blocks (version, flags, counts, timestamps, platform). |
| `PVER` | [`PackageVersion`](../src/EvilHop/Blocks/Package/PackageVersion.cs) | The archive format version and client version. |
| `PFLG` | [`PackageFlags`](../src/EvilHop/Blocks/Package/PackageFlags.cs) | A bitmask of platform, region, and language flags. |
| `PCNT` | [`PackageCount`](../src/EvilHop/Blocks/Package/PackageCount.cs) | Asset and layer counts, plus the largest asset/layer sizes. |
| `PCRT` | [`PackageCreated`](../src/EvilHop/Blocks/Package/PackageCreated.cs) | The archive's creation timestamp. |
| `PMOD` | [`PackageModified`](../src/EvilHop/Blocks/Package/PackageModified.cs) | The archive's last-modified timestamp. |
| `PLAT` | [`PackagePlatform`](../src/EvilHop/Blocks/Package/PackagePlatform.cs) | Platform, region, and language strings. Introduced in Battle for Bikini Bottom; absent from N100F. |
| `DICT` | [`Dictionary`](../src/EvilHop/Blocks/Dictionary/Dictionary.cs) | Root of the asset table and layer table. |
| `ATOC` | [`AssetTable`](../src/EvilHop/Blocks/Dictionary/AssetTable.cs) | The list of every asset's `AHDR` entry. |
| `AINF` | [`AssetInf`](../src/EvilHop/Blocks/Dictionary/AssetInf.cs) | A single unknown scalar on `ATOC`. |
| `AHDR` | [`AssetHeader`](../src/EvilHop/Blocks/Dictionary/AssetHeader.cs) | One asset's ID, type, offset, size, and flags. |
| `ADBG` | [`AssetDebug`](../src/EvilHop/Blocks/Dictionary/AssetDebug.cs) | One asset's alignment, name, source filename, and checksum. |
| `LTOC` | [`LayerTable`](../src/EvilHop/Blocks/Dictionary/LayerTable.cs) | The list of every layer's `LHDR` entry. |
| `LINF` | [`LayerInf`](../src/EvilHop/Blocks/Dictionary/LayerInf.cs) | A single unknown scalar on `LTOC`. |
| `LHDR` | [`LayerHeader`](../src/EvilHop/Blocks/Dictionary/LayerHeader.cs) | One layer's type and the asset IDs it lists. |
| `LDBG` | [`LayerDebug`](../src/EvilHop/Blocks/Dictionary/LayerDebug.cs) | A single unknown scalar on `LHDR`. |
| `STRM` | [`AssetStream`](../src/EvilHop/Blocks/AssetStream/AssetStream.cs) | Root of the asset data blocks. |
...
- **Managed / locked fields** - fields of `AHDR`, `ADBG`, `LHDR`, and `DPAK` that an
  [`AssetSession`](../src/EvilHop/Assets/Session/AssetSession.cs) locks while it owns those blocks, so an
...
- **AssetId** - a 32-bit reference to an asset, normally the BKDR hash of its name. See
  [`AssetId`](../src/EvilHop/Common/AssetId.cs).
...
- **Session** - the scoped [`AssetSession`](../src/EvilHop/Assets/Session/AssetSession.cs) that owns an
...
- **Diagnostic** - a non-fatal problem recorded on `AssetSession.Diagnostics` when an asset fails to
  ...
  [`AssetDiagnostic`](../src/EvilHop/Assets/Session/AssetDiagnostic.cs).
...
- **Fill byte / gap** - the byte value (sampled from the archive's own padding, or a default) used
  ...
  [`AssetSession.GapBytesFor`](../src/EvilHop/Assets/Session/AssetSession.cs).
