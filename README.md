...
- **Untyped** - no codec exists yet. The asset reads through its shape's generic prefix if available and remains byte-preserved. Not yet supported: `Dynamic`, `NavigationMesh`, `Portal`, `SceneSettings`, `SlideProperty`, `Spline`, `VillainProperties`, `Volume`, `Wireframe`, `ZipLine`.
